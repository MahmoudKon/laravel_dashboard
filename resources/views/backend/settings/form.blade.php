...
<div class="row">
    ...
    <div class="col-md-6">
        <div class="form-group">
            <label class="required">@lang('inputs.select-data', ['data' => trans('menu.input_type')])</label>
            <select class="select2 form-control" name="input_type_id" id="input_type_id" data-placeholder="--- @lang('inputs.select-data', ['data' => trans('menu.content_type')]) ---" >
                <option value="">@lang('inputs.please-select')</option>
                @foreach ($types as $id => $name)
                    <option value="{{ $id }}" @selected(isset($row) && $row->input_type_id == $id || old('input_type_id') == $id)>{{ $name }}</option>
                @endforeach
            </select>
            <x-validation-error input='input_type_id' />
        </div>
    </div>
    {{-- END CONTENT TYPE --}}
</div>
    ...
    <script>
        $(function() {
            $(document).ready(function() {
                if ($('#input_type_id').val() > 0) { $('#input_type_id').change() }
            });

            let input_type = $('#load-content-type-input');
            let content_id = "{{ $row->id ?? null }}";
            $('body').on('change', '#input_type_id', function() {
                let url = "{{ routeHelper('settings.type.input') }}";
                let input_type_id = $(this).val();
                input_type.addClass('load').fadeOut(300, function () { $(this).empty() });

                $.ajax({
                    url: url,
                    type: "POST",
                    data: {input_type_id: input_type_id, content_id: content_id},
                    success: function (response, textStatus, jqXHR) {
                        input_type.fadeIn(300, function() { $(this).html(response); });
                    },
                    error: function (jqXHR, textStatus, errorMessage) {
                        handleErrors(jqXHR);
                    },
                    complete: function() { input_type.removeClass('load'); }
                });
