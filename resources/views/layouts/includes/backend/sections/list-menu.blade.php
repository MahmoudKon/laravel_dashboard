...
<li class="nav-item {{ $menu->visibleSubs->Count() ? "has-sub" : "" }} {{ activeMenu($menu->route) }}" data-route="{{ ROUTE_PREFIX.$menu->route }}">
    <a href="{{ $menu->route && Route::has(ROUTE_PREFIX.$menu->route) ? routeHelper($menu->route) : "#"  }}" style="color: {{ $menu->color }} !important">
